DROP DATABASE IF EXISTS retails_new;

CREATE DATABASE retails_new;
...
CREATE TABLE users(
    ...
    user_unique_id VARCHAR(20),
    user_phone_no VARCHAR(30),
    user_dob DATE,
...
WHERE CAST(user_id AS CHAR) = 'user_id'
   OR user_first_name = 'user_first_name'
   OR user_last_name = 'user_last_name'
   OR user_email_id = 'user_email_id';
...
-- Exercise 2: Tên ngày sinh của user sinh trong tháng 6
SELECT 
    user_id,
    user_dob,
    user_email_id,
...
SELECT 
    REGEXP_SUBSTR(REPLACE(user_phone_no, '+', ''), '^[0-9]+') AS country_code,
    COUNT(*) AS user_count
FROM users
WHERE user_phone_no IS NOT NULL
GROUP BY country_code
ORDER BY CAST(country_code AS UNSIGNED);
